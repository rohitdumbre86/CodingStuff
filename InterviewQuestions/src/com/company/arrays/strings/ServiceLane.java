 ...
 * three vehicles can pass through the kth segment. Given the entry and exit
 * point of Calvin's vehicle in the service lane, output the type of the largest
 * vehicle which can pass through the service lane (including the entry and exit
 * segments).
 * 
 * Input Format
 * 
...
public class ServiceLane {
	...
	public int returnMaxVehicle(int[] value, int startIndex, int endIndex) {
		int minValue = 3;
	...
	public static void main(String[] args) {
		...
		int serviceSize = in.nextInt();
		int startIndex = 0;
		int[] totalWidth = new int[serviceSize];
		int numberOfTestCases = in.nextInt();

		while (in.hasNext() && startIndex < serviceSize) {
			int width = in.nextInt();
			totalWidth[startIndex] = width;
			startIndex++;
		}
		...
		for (int k = 1; k <= numberOfTestCases; k++) {
			int i = in.nextInt();
			int j = in.nextInt();
			System.out.println(s.returnMaxVehicle(totalWidth, i, j));
		}
